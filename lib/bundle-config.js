 ...
 */
function BundleConfig(config, ctx) {
    this._config = config;
    this._ctx = ctx;
...
BundleConfig.prototype.prepare = function () {
    ...
    if (!modules || !Array.isArray(modules)) {
        ...
        var includeModulePatterns = ab.expand(this._config.include || []);
        var excludeModulePatterns = ab.expand(this._config.exclude || []);
    ...
    allModules.forEach(function (moduleId) {
        this._patterns.forEach(function (pattern) {
            ...
            if (pattern[0] === '!') {
                ...
                if (edp.path.satisfy(moduleId, pattern)) {
