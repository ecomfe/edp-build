...
ManifestCompiler.prototype.process = function ( file, processContext, callback ) {
    ...
    if( !this.fileCount ) {
        ...
        this.manifests.forEach( function ( option ) {
            ...
            // 按manifest文件名，存入信息
            _this.manifestInfo[ manifestName ] = {
                // 访问文件总数
                visitFilesCount: 0,
                // manifest版本号
                version: null,
    ...
    this.manifests.forEach( function ( option ) {
        ...
        if ( cachePage && pathSatisfy( file.path, cachePage ) ) {
            ...
            addManifestToProcessContext( processContext , fullPath, manifestName );

            network = eachArray( network, valueReplacer );
            fallback = eachArray( fallback, valueReplacer );
            ...
            if ( cache && cache.length > 0) {
                cache = eachArray( cache, valueReplacer );
 ...
 * @inner
 * {array} arr 数据数组
 * @return {string} 用换行符连接的字符串
 */
function joinArrayWithWrap( arr ) {
    return arr.join( '\n');
}
/**
 * 给manifest文件中加入资源
    ...
    if( manifest ) {
        for( var key in manifestInfo ) {
            ...
            if( '[object Array]' == Object.prototype.toString.call( value ) ) {
                manifestInfo[ key ] = joinArrayWithWrap( value );
            }
        }
 ...
 * {string} manifestName manifest的文件名
 */
function addManifestToProcessContext( processContext, fullPath, manifestName ) {

    var fullPath = path.resolve( path.dirname( fullPath ), manifestName );
    ...
    var fileData = new FileInfo( {
        data         : joinArrayWithWrap( createManifest() ),
        extname      : extname,
        path         : relativePath,
        fullPath     : fullPath,
        stat         : {},
    ...
    if ( path.extname( outputPath ) == '.css' ) {
        ...
        manifestInfo.css.forEach( function ( href ) {
            if( outputPath === href ) {
                ...
                var content = file.data;
                var imgs = require( '../util/get-css-url' )( content, replacer );
                if( imgs.length > 0 ) {
                    Array.prototype.push.apply( manifestInfo.image , imgs );
                }
            }
        } );
