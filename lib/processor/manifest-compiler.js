...
ManifestCompiler.prototype.process = function ( file, processContext, callback ) {
    ...
    this.manifests.forEach( function ( manifestData, index ) {
        ...
        if ( pathSatisfy( file.path, cachePage ) ) {
            if ( cachePage ) {
                // 把缓存页面的数据添加到data中
                var content = getCachePage( file.data, manifestName );
            ...
            // manifest文件全路径
            var fullPath = path.resolve( fileDir, manifestName );
            ...
            if ( manifest.autoCache === true ) {
                var resourceData = getResource( 
                    file, 
                    _this, 
                    processContext, 
                    cachePage
                );
                whiteTypes.forEach( function( resType ) {
    ...
    if ( this.visitCount === this.fileCount ) {
        ...
        relativePaths.forEach( function ( relativePath ) {
            for (var key in _this.manifestInfo) {
                var manifest = _this.manifestInfo[ key ];
                var module = modluePathReplacer( 
                    processContext, 
                    relativePath, 
        ...
        for (var key in _this.manifestInfo ) {
            var manifest = _this.manifestInfo[ key ];
            whiteTypes.forEach( function( resType ) {
                var value = manifest[ resType ];
                if ( Array.isArray( value ) ) {
                    manifest[ resType ] = uniqueArray( value );
                }
            });
            // console.log(manifest)
 ...
 * @return {object} 格式化后的资源
 */
function getResource( file, process, processContext, cachePage ) {
    var manifest = process.manifestInfo[ cachePage ];
    if ( manifest ) {
        var data = manifest.data;
        var content = file.data;
        var cache = manifest.cache;
        var pageResource = getPageResource( content, process.configFile );
        return {
            html: [ file.outputPath ],
            image: pageResource.image,
            css: pageResource.css,
            script: pageResource.script,
            moduleSource: pageResource.moduleSource
        };
    }
}
/**
 ...
 * @return {object} 资源对象
 */
function getPageResource( content, configFile ) {
    // 获取模块id数组
 ...
 */
function addCssUrl ( processContext, relativePath, css ) {
    var result = [];
...
function setResources( processContext , manifestName , data ) {
    ...
    if( manifest ) {
        ...
        for( var key in data ) {
            var value = data[ key ];
            if( whiteTypes.indexOf( key ) !== -1 && Array.isArray( value ) ) {
                data[ key ] = value.join( '\n' );
            }
        }
