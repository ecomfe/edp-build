...
function buildPage( content , name ) {
    ...
    var manifest = ''
                 ...
                 + '" >';
    if( content ) {
        return content.replace( reg , function ( whole, matchAttr , gt ) {

            return whole.replace(gt, manifest);
...
function getResources(content) {
    ...
    requireJsList = matchAttr( requireJsMatch );
    ...
    function matchAttr( matchList ) {
        var list = [];
        matchList.forEach( function ( item ) {
            list.push( item.match(/\'(.*)\'/)[1] );
        });
        return list;
    }
