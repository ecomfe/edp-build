...
ManifestCompiler.prototype.process = function (file, processContext, callback) {
    ...
    this.manifests.forEach(function (manifestData, index) {
        ...
        // 判断路径片段是否满足规则
        if (helper.satisfy(file.path, cachePage)) {
            ...
            if (manifest.autoCache === true) {
                ...
                for (var resType in resourceData) {
                    if (!resourceData.hasOwnProperty(resType)) {
                        continue;
                    }
    ...
    if (this.visitCount === this.fileCount) {
        ...
        relativePaths.forEach(function (relativePath) {
            for (var key in manifestInfo) {
                if (!manifestInfo.hasOwnProperty(key)) {
                    continue;
                }
...
function getCssUrlPath(processContext, relativePath, css) {
    ...
    css.forEach(function (item) {
        if (file && helper.satisfy(item, file.outputPath)) {
...
function setResources(processContext, manifestName, data) {
    ...
    if (manifest) {
        for (var key in data) {
            ...
            var value = data[key];
            if (manifestArrayKeys.indexOf(key) !== -1 && Array.isArray(value)) {
