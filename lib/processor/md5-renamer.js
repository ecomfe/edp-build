...
var kDefaultHtmlFiles = ['*.html', '*.htm', '*.phtml', '*.tpl', '*.vm'];
var kDefaultCssFiles = ['*.styl', '*.css', '*.less'];
var kDefaultJsFiles = ['*.js'];
    ...
    options = edp.util.mix({
        ...
        replacements: {
            ...
            html: {
                ...
                files: kDefaultHtmlFiles
            },
            // 处理url(...)
            css: {
                files: kDefaultCssFiles
            },
            // 处理require.toUrl()
            js: {
                files: kDefaultJsFiles
            }
        },
...
MD5Renamer.prototype.process = function (file, processContext, callback) {
    for (var type in this.replacements) {
        ...
        match = replacement.files.some(function(pattern) {
            return require('../helper').satisfy(file.path, pattern);
        });
        /* jshint ignore:end */
