...
TplMerge.prototype.afterAll = function (processContext) {
    ...
    var outputModuleId = this.outputModuleId || fileName;

    function replacer(resourceId) {
        if (outputAsAmdModule) {
            return outputPluginId + '!' + outputModuleId;
        }
...
TplMerge.prototype._appendOutput = function (processContext, data, extname) {
    ...
    if (this.outputAsAppend) {
        var fileData = processContext.getFileByPath(this.outputPath);

        if (!fileData) {
            this.log.fatal('No file to append, file = [%s]', file.outputPath);
            return null;
        }

        fileData.setData(fileData.data + '\n' + data);
        return fileData;
    }
    else {
