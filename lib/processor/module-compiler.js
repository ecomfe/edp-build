 ...
 * @return {Object}
 */
ModuleCompiler.prototype.getDependenceInfo = function () {
    return this.depMap;
};
...
ModuleCompiler.prototype.process = function (file, processContext, callback) {
    ...
    var module;
    var combineModuleIds = [];
    var configFile = edp.path.resolve(processContext.baseDir, this.configFile);
    // [ packages, paths, baseUrl ]
    var moduleIds = helper.getModuleId(file.fullPath, configFile);
    if (moduleIds.length === 1) {
        module = new Module(moduleIds[0], this.compilerContext, null, combineModuleIds);
    }
    ...
    if (module) {
        ...
        var combineModuleFiles = module.getCombineModuleIds().map(function (id) {
            return edp.amd.getModuleFile(id, configFile);
        });
        combineModuleFiles = combineModuleFiles.filter(function (item) {
            return !!item;
        });
        combineModuleFiles.length && (this.depMap[file.path] = combineModuleFiles);
    }
