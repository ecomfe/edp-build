 ...
 * @param {Object} bundleConfigs combine字段的内容.
 */
function CompilerContext (pctx, moduleConfig, reader, bundleConfigs) {
    this._pctx = pctx;
     ...
     */
    this._bundleConfigs = bundleConfigs;
...
CompilerContext.prototype.getBundleConfig = function (moduleId) {
    if (!(moduleId in this._bundleConfigs)) {
        return null;
    }

    var bundleConfig = this._bundleConfigs[moduleId];
    if (!bundleConfig) {
    ...
    if (!(bundleConfig instanceof BundleConfig)) {
        bundleConfig = new BundleConfig(bundleConfig, this);
        this._bundleConfigs[moduleId] = bundleConfig;
    }
