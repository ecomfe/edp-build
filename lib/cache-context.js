...
function wrapProcessor(processor, cacheContext) {
    ...
    processor.beforeAll = function () {
        ...
        for (var i = processFiles.length - 1; i >= 0; i--) {
            ...
            cacheContext.sourceFiles.push(file);
            ...
            if (u.isFunction(processor.hasChanged)
                ...
                u.isEmpty(cacheContext.manifest)
 ...
 * @return {Array.<Object>}
 */
CacheContext.prototype.analyzeDependence = function (file, processContext) {
    var deps = [];
    var processor = this.processor;
    var getDependence = processor.getDependenceInfo;
    var depMap;
    if (u.isFunction(getDependence)) {
        depMap = getDependence.call(processor);
    }
    ...
    var imports = depMap[file.path] || [];
    imports.forEach(function (url) {
        if (edp.path.isLocalPath(url)) {
            var relativePath = edp.path.relative(processContext.baseDir, url);
            var depFile = processContext.getFileByPath(relativePath);
            depFile && deps.push({
                path: relativePath,
                md5: depFile.rawMd5
            });
...
CacheContext.prototype.serializeManifiest = function () {
    var me = this;
    var processContext = me.processContext;
    var sourceFiles = me.sourceFiles;
    var sourceMap = {};

    sourceFiles.forEach(function (file) {
        var result = {md5: file.rawMd5};

        var deps = me.analyzeDependence(file, processContext) || [];
    ...
    fs.writeFileSync(
        me.manifestPath, JSON.stringify(sourceMap, null, 4), 'UTF-8'
    );
};
