...
CacheContext.prototype.initCacheProcessResult = function (file) {
    ...
    if (!cacheInfo) {
        edp.log.fatal('%s cache information is changed unexpected, please delete cache result manually and execute edp build again!', file.path);
        return;
    }
...
CacheContext.prototype.readCacheContent = function (path) {
    ...
    try {
        ...
        edp.log.fatal('Read processor %s cache file %s fail!', this.processor.name, cachePath);
    }
};
...
CacheContext.prototype.addSourceFile = function (file) {
    ...
    var deps = (u.isFunction(analyzeDeps) && analyzeDeps.call(this.processor, file)) || [];
...
CacheContext.prototype.updateCache = function () {
    edp.log.info('cache processor %s process result information...', this.processor.name);
...
CacheContext.prototype.serializeManifiest = function () {
    ...
    fs.writeFileSync(this.manifestPath, JSON.stringify(sourceMap, null, 4), 'UTF-8');
};
...
CacheContext.prototype.loadManifest = function (processContext) {
    edp.log.info('load processor %s cache manifest information...', this.processor.name);
