...
function compileModule( code, moduleId, moduleConfig, combine, opt_excludeModules ) {
    ...
    if ( combine ) {
        ...
        var files = null;
        if ( !combine.files || !Array.isArray( combine.files ) ) {
            // 如果存在include和exclude，那么 files = include.concat( exclude )
            var includeModulePatterns = ab.expand( combine.include || [] );
            var excludeModulePatterns = ab.expand( combine.exclude || [] );
            files = includeModulePatterns.concat(
