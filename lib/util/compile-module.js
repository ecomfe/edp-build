...
function compileModule( code, moduleId, moduleConfig, combine, opt_excludeModules ) {
    ...
    if ( combine ) {
        ...
        //
        // FIXME(user) 如果遇到了模块别名的情况，应该如何处理呢？
        ...
        if ( excludeModulePatterns.length ) {
            ...
            allModules = allModules || edp.esl.getAllModules( moduleConfig );
        ...
        if ( moduleId !== moduleInfo.id ) {
            // console.log( '%s => %s', moduleInfo.id, JSON.stringify( Object.keys( excludeModules ) ) );
            // edp.log.warn( '%s != %s', moduleId, moduleInfo.id );
