 ...
 *
 * @param {ModuleInfo} moduleInfo 入口模块的信息
 * @param {{fileset: Array, exclude: Array, include: Array}} moduleSet 模块的集合.
 *
 * @return {Object}
 */
function initializeExcludeMap( moduleInfo, moduleSet ) {
    ...
    var fileset = moduleSet.fileset;
    if ( fileset && fileset.length ) {
        fileset.forEach(function( depId ){
...
function compileModule( code, moduleId, moduleConfig, combine, opt_excludeModules ) {
    ...
    if ( combine ) {
        ...
        if ( combine.fileset && Array.isArray( combine.fileset ) ) {
            ...
            fileset = edp.glob.filter( combine.fileset, allModules );
        }
        else {
        ...
        var excludeModules = opt_excludeModules ||
            initializeExcludeMap( moduleInfo, {
                fileset: fileset,
