...
/**
 * _ALL_MODULES_CACHE是用来缓存结果的，保证不同的文件如果需要
 * combine的时候，只需要整体扫描一次.
 *
 * @return {Array.<string>}
 */
var _ALL_MODULES_CACHE = null;
function getAllModules(moduleConfig) {
    return _ALL_MODULES_CACHE || (_ALL_MODULES_CACHE = edp.amd.getAllModules(moduleConfig));
}

var _EXPLICIT_MODULES_CACHE = {};

/**
 * @param {Array.<string>} patterns 用来当做key.
 * @param {string} moduleConfig module.conf的路径.
 */
function getExplicitModulesByPatterns(patterns, moduleConfig) {
    var key = patterns.join(',');
    if (_EXPLICIT_MODULES_CACHE[key]) {
        return JSON.parse(_EXPLICIT_MODULES_CACHE[key]);
    }
    ...
    var value = [
        explicitExcludeModules, explicitIncludeModules
    ];

    _EXPLICIT_MODULES_CACHE[key] = JSON.stringify(value);
...
function compileModule(code, moduleId, moduleConfig, combine, excludeModules) {
    ...
    if (combine) {
        ...
        // 如果allModules和patterns没有变化的话，那么初始化出来的
        // explicitExcludeModules 和 explicitIncludeModules 应该是一样的
        // 所以把结果放到缓存里面去，减少调用 edp.path.satisfy 的开销

        var explicitModules = getExplicitModulesByPatterns(patterns, moduleConfig);

        var explicitExcludeModules = explicitModules[0];
        var explicitIncludeModules = explicitModules[1];
